...
const EditCodingContestForm = () => {
    ...
    const getContestById = async () => {
        try {
            ...
            // Ensure `categories` exists in formData
            setFormData({
                ...data,
                categories: data.category || [], // Convert category to categories
            });
        } catch (error) {
            console.log(error);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`${API}/api/problem/update-coding-contest/${params.id}`, updatedFormData, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: authorizationToken,
                },
...
export default EditCodingContestForm;