...
export default function ContestistPage() {
    ...
    const getExams = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${API}/api/problem/get-all-contest-user`, {
                headers: {
                            ...
                            <table className="min-w-full leading-normal">
                                ...
                                <tbody>
                                    {filteredExams.map((exam, index) => {
                                        ...
                                        const isExamOngoing =
                                            (currentTime >= examStartTime && currentTime <= examEndTime) && !exam?.ended;
                                        const isExamScheduled = currentTime < examStartTime;
                                        const isExamEnded = (currentTime > examEndTime) || (exam?.ended && !isExamScheduled);

                                        const status = isExamScheduled ? "Upcoming" : isExamEnded ? "Completed" : isExamOngoing ? "On Going" : "";
                                        return (
                                            <tr key={index} className="hover:bg-gray-50" >
                                                <td className="px-5 py-5 border-b border-gray-200 text-sm">
                                                    <p className="text-gray-900 whitespace-no-wrap font-medium">{exam.name}</p>
                                                </td>
                                                <td className="px-5 py-5 border-b border-gray-200 text-sm">
                                                ...
                                                <td className="px-5 py-5 border-b border-gray-200 text-sm">
                                                    ...
                                                    {
                                                        isExamOngoing && (
                                                            <Link to={`/user/coding-contest/${exam?._id}`}>
                                                                <button
                                            ...
                                            isExamOngoing && (
                                                <Link to={`/user/coding-contest/${exam?._id}`}>
                                                    <button
                                                        className={`${getButtonColor(status)} text-white font-bold py-2 px-4 rounded-full transition duration-300 ease-in-out flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500`}
                                                        disabled={status === "Completed"}
                                                        aria-label={`Take ${exam.name} exam`}
                                                    >
                                                        <MdPlayCircleOutline className="mr-2" />
                                                        Take Exam
                                                    </button>
